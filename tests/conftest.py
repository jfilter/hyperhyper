"""
Shared sample data and fixtures for the test suite.

The two sample texts used to live, copy-pasted verbatim, in four different
places; they are defined here once.
"""
...
SOME_TEXTS = [SOME_TEXT1, SOME_TEXT2]


@pytest.fixture()
def sample_texts():
    """
    The raw sample texts, repeated a couple of times to get a corpus that is
    big enough to be worth counting.
    """
    return SOME_TEXTS * 10
...
def corpus():
    ...
    Uses the pure-Python tokenizer instead of the default `texts_to_sents` so
    that the fast test suite neither imports spaCy nor needs its model.
    """
    return hyperhyper.Corpus.from_texts(SOME_TEXTS * 10, preproc_func=tokenize_texts)


@pytest.fixture()
def corpus_on_disk(corpus, tmp_path):
    """
    Like `corpus`, but with the texts already chunked out to disk, which is
    what `count_pairs` reads.
    """
    corpus.texts_to_file(tmp_path / "texts", 5)
    return corpus


@pytest.fixture()
def bunch_path(tmp_path):
    """
    Where a `Bunch` should put its files during a test.

    A relative path would scatter pickles, matrices and a `results.db` all over
    the repository root on every run.
    """
    return tmp_path / "test_bunch"


@pytest.fixture()
def text_files(tmp_path):
    """
    A directory of `.txt` files, as consumed by `Corpus.from_text_files`.
    """
    directory = tmp_path / "text_files"
    directory.mkdir()
    for i, text in enumerate(SOME_TEXTS * 10):
        (directory / f"{i}.txt").write_text(text, encoding="utf-8")
    return directory
...
def corpus_factory():
    """
    Build a *fresh* corpus per call.

    Sharing one corpus across bunches is supported (see
    `test_one_corpus_feeds_two_bunches`), but a fresh corpus keeps tests
    independent of the chunk files an earlier bunch left behind.
    """

    def make():
        return hyperhyper.Corpus.from_texts(
            SOME_TEXTS * 10, preproc_func=tokenize_texts
        )
...
def spacy_model():
    """
    Skip the test unless spaCy *and* the English model are actually installed.
    """
    spacy = pytest.importorskip("spacy")
    if not spacy.util.is_package(SPACY_MODEL):
        pytest.skip(f"the spaCy model {SPACY_MODEL} is not installed")
    return SPACY_MODEL
